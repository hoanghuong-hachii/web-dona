...
$(document).ready(function() {
   // console.log(env_Url);
    ...
    const idCategory = urlParams.get('category');
    const nameTitle = urlParams.get('title');
    
    console.log(idCategory)
    $('.bread-title').text(nameTitle)
   ...
   fetchData(currentPage);
    ...
    function fetchData(page) {
       ...
       // console.log('gia: '+priceFrom+ ' - '+ priceTo)
        $.ajax({
            url: `${env_Url}/api/v1/Products/roleUser/filter?page=${page}&size=12`,
            type: 'POST',
            ...
            success: function(response) {
                console.log(' pages:', response.content);
    ...
    function updateUI(products, priceFrom, priceTo) {
        ...
        products.forEach(product => {
            ...
            console.log(' gia' + product)

            if(price_pro >= priceFrom && price_pro <= priceTo) {
                var productHtml = `
